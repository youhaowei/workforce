...
describe("WorkflowService", () => {
  ...
  describe("update", () => {
    ...
    it("should throw for non-existent workflow", async () => {
      const service = freshService();
      await expect(service.update(WS_ID, "wf_fake", { name: "X" })).rejects.toThrow(
        "Workflow not found",
      );
      service.dispose();
    });
  ...
  describe("list", () => {
    it("should list non-archived workflows", async () => {
      ...
      const wf3 = await service.create("ws_list", {
        name: "WF3",
        description: "",
        steps: [step("s1")],
      });
  ...
  describe("validate", () => {
    ...
    it("should detect cycles in complex DAG", () => {
      ...
      const result = service.validate({
        name: "Complex Cycle",
        steps: [step("a", "agent", ["c"]), step("b", "agent", ["a"]), step("c", "agent", ["b"])],
      });
      expect(result.valid).toBe(false);
  ...
  describe("getExecutionOrder", () => {
    ...
    it("should throw for non-existent workflow", async () => {
      const service = freshService();
      await expect(service.getExecutionOrder(WS_ID, "wf_nonexistent")).rejects.toThrow(
        "Workflow not found",
      );
      service.dispose();
    });
