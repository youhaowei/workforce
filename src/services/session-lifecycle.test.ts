...
describe("session-lifecycle", () => {
  ...
  describe("createWorkAgent", () => {
    ...
    it("passes workflow metadata", async () => {
      const deps = makeDeps();
      const session = await createWorkAgent(
        deps,
        makeConfig({
          workflowId: "wf-1",
          workflowStepIndex: 2,
  ...
  describe("transitionState", () => {
    ...
    it("transitions active → completed with completionSummary", async () => {
      ...
      const lifecycle = completed.metadata.lifecycle as {
        state: string;
    ...
    it("throws on invalid transition", async () => {
      ...
      await expect(transitionState(deps, session.id, "completed", "skip")).rejects.toThrow(
    ...
    it("throws for terminal states", async () => {
      ...
      await expect(transitionState(deps, session.id, "active", "restart")).rejects.toThrow(
    ...
    it("throws if session not found", async () => {
      const deps = makeDeps();
      await expect(transitionState(deps, "missing", "active", "start")).rejects.toThrow(
    ...
    it("writes meta record with updated lifecycle", async () => {
      ...
      expect(deps.writeRecords).toHaveBeenCalledWith(session.id, [
  ...
  describe("listByState", () => {
    it("returns sessions matching the state", async () => {
      ...
      sessions.set(
        "s1",
        makeSession("s1", { metadata: { lifecycle: { state: "active", stateHistory: [] } } }),
      );
      sessions.set(
        "s2",
        makeSession("s2", { metadata: { lifecycle: { state: "completed", stateHistory: [] } } }),
      );
      sessions.set(
        "s3",
        makeSession("s3", {
          metadata: { lifecycle: { state: "active", stateHistory: [] }, orgId: "org-1" },
        }),
      );
    ...
    it("filters by orgId when provided", async () => {
      ...
      sessions.set(
        "s2",
        makeSession("s2", {
          metadata: { lifecycle: { state: "active", stateHistory: [] }, orgId: "org-2" },
        }),
      );
