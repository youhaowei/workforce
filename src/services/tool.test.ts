...
describe('ToolService', () => {
  ...
  describe('register / unregister', () => {
    ...
    it('throws on duplicate registration', () => {
      const svc = getToolService();
      svc.register('echo', async (args) => args);
      expect(() => svc.register('echo', async (args) => args))
  ...
  describe('execute', () => {
    ...
    it('catches handler errors and returns error result', async () => {
      const svc = getToolService();
      svc.register('fail', async () => { throw new Error('boom'); });
    ...
    it('catches non-Error throws', async () => {
      const svc = getToolService();
      svc.register('fail', async () => { throw 'string error'; });
    ...
    it('emits ToolStart and ToolEnd events on success', async () => {
      ...
      expect(events[1]).toMatchObject({ type: 'ToolEnd', toolName: 'echo', result: { text: 'hi' } });
    });
