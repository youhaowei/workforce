...
function tc(name: string, input: Record<string, unknown> = {}, seq = 0): JournalToolCall {
  return { t: 'tool_call', seq, ts: Date.now(), actionId: `tc-${seq}`, messageId: `msg-${seq}`, name, input };
}
...
describe('extractPlansFromRecords', () => {
  ...
  it('falls back to filename for title when no H1 heading', () => {
    const records: JournalRecord[] = [
      tc('EnterPlanMode', {}, 1),
      tc('Write', { file_path: '/plans/refactor-auth.md', content: 'No heading here\n\nJust text' }, 2),
      tc('ExitPlanMode', {}, 3),
    ];
  ...
  it('skips non-tool_call records', () => {
    const records: JournalRecord[] = [
      { t: 'message', seq: 0, ts: Date.now(), id: 'msg-1', role: 'user', content: 'plan something' },
      tc('EnterPlanMode', {}, 1),
      { t: 'message_final', seq: 2, ts: Date.now(), id: 'msg-2', role: 'assistant', content: 'writing plan', stopReason: 'tool_use' },
