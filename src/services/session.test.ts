...
describe("SessionService", () => {
  ...
  describe("updateSession", () => {
    ...
    it("should throw when saving a deleted/untracked session", async () => {
      ...
      await expect(service.updateSession(session.id, { title: "Ghost" })).rejects.toThrow(
        "Session not found",
      );
    });
  });
  ...
  describe("fork", () => {
    ...
    it("should throw for invalid message index", async () => {
      ...
      await expect(service.fork(parent.id, { atMessageIndex: 5 })).rejects.toThrow(
        "Invalid message index",
      );
      await expect(service.fork(parent.id, { atMessageIndex: -2 })).rejects.toThrow(
        "Invalid message index",
      );
    });
  ...
  describe("transitionState", () => {
    it("should transition created → active", async () => {
      ...
      const lifecycle = updated.metadata.lifecycle as {
        state: string;
    ...
    it("should reject transitions from terminal states", async () => {
      ...
      await expect(service.transitionState(session.id, "active", "Try again")).rejects.toThrow(
    ...
    it("legacy meta-patched parentId should survive replay (backward compat)", async () => {
      ...
      const lines = [
        ...
        JSON.stringify({
          t: "meta",
          updatedAt: 2,
          patch: { parentId: "sess_old_parent" },
        }),
      ];
      await writeFile(join(dir, `${sessionId}.jsonl`), lines.join("\n") + "\n", "utf-8");
  ...
  describe("lazy loading", () => {
    ...
    it("search() should find messages after lazy load", async () => {
      ...
      await service.recordMessage(s.id, {
        id: "msg_s",
        role: "user",
        content: "unique_search_keyword_xyz",
        timestamp: Date.now(),
      });
    ...
    it("replaySessionMetadata should read only the header line", async () => {
      ...
      const lines = [
        ...
        JSON.stringify({
          ...
          seq: 2,
          ts: 700,
          patch: { title: "Post-Consolidation Rename" },
        }),
      ];
      await writeFile(join(dir, `${sessionId}.jsonl`), lines.join("\n") + "\n", "utf-8");
  ...
  describe("consolidation duplicate prevention", () => {
    ...
    it("concurrent recordMessage calls should not produce duplicates", async () => {
      ...
      await Promise.all([
        service.recordMessage(session.id, {
          id: "msg_c1",
          role: "user",
          content: "Msg 1",
          timestamp: Date.now(),
  ...
  describe("hydration status", () => {
    ...
    it("background rehydration should fix stale metadata from header-only replay", async () => {
      ...
      const session =
        service.getCurrent() ?? (await service.list()).find((s) => s.id === sessionId);
