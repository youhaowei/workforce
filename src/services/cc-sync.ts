...
export function importCCSession(deps: CCSyncDeps, ccFilePath: string, orgId?: string): Promise<Session> {
  const existing = importFlights.get(ccFilePath);
  if (existing) return existing;
  const promise = doImportCCSession(deps, ccFilePath, orgId).finally(() => importFlights.delete(ccFilePath));
  importFlights.set(ccFilePath, promise);
  return promise;
}

async function doImportCCSession(deps: CCSyncDeps, ccFilePath: string, orgId?: string): Promise<Session> {
  ...
  const existing = [...deps.sessions.values()].find((s) => s.metadata.ccSessionId === result.header.id);
  if (existing) { log.info({ ccSessionId: result.header.id }, 'CC session already imported'); return existing; }
  ...
  const header: JournalRecord = {
    t: 'header', v: JSONL_VERSION, seq: 0, ts: now,
    id: session.id, title: session.title,
    createdAt: session.createdAt, metadata: session.metadata,
  };
...
export async function checkCCSync(deps: CCSyncDeps, sessionId: string): Promise<{ inSync: boolean }> {
  ...
  } catch { return { inSync: true }; }
}

export async function checkCCSyncBatch(deps: CCSyncDeps, sessionIds: string[]): Promise<Record<string, boolean>> {
  await deps.ensureInitialized();
  const entries = await Promise.all(sessionIds.map(async (id) => [id, (await checkCCSync(deps, id)).inSync] as const));
...
async function doSyncCCSession(deps: CCSyncDeps, sessionId: string): Promise<Session> {
  ...
  if (result && result.records.length > 0) {
    ...
    const newLineCount = lastSyncedLines + result.stats.totalCCRecords + result.stats.malformedLines;
    ...
    const now = Date.now();
    session.metadata.ccLastSyncedAt = now;
    await deps.writeRecords(sessionId, [{ t: 'meta', seq: 0, ts: now, patch: { ccLastSyncedAt: now } } satisfies JournalMeta]);
  }
