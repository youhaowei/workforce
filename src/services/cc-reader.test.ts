...
describe("cc-reader", () => {
  ...
  describe("user records", () => {
    ...
    it("maps tool-result-only user messages to meta (not message)", async () => {
      ...
      await writeFile(
        f,
        ccUser([{ type: "tool_result", tool_use_id: "tu-1", content: "result data here" }]),
        "utf-8",
      );
      const result = await readCCSession(f);
  ...
  describe("assistant records", () => {
    ...
    it("preserves usage from last assistant record", async () => {
      const f = nextFile();
      const lines = [ccAssistant("msg-1", [{ type: "text", text: "hi" }])].join("\n");
      await writeFile(f, lines, "utf-8");
      const result = await readCCSession(f);
  ...
  describe("malformed lines", () => {
    it("skips malformed lines and counts them", async () => {
      const f = nextFile();
      const lines = ["{broken json", ccUser("valid message"), "also broken}"].join("\n");
      await writeFile(f, lines, "utf-8");
      const result = await readCCSession(f);
  ...
  describe("full multi-turn session", () => {
    it("handles a realistic multi-turn conversation", async () => {
      ...
      const lines = [
        ...
        ccAssistant("msg-1", [
          { type: "tool_use", id: "tu-read", name: "Read", input: { path: "/src/session.ts" } },
        ]),
        ...
        ccAssistant("msg-2", [
          { type: "tool_use", id: "tu-edit", name: "Edit", input: { path: "/src/session.ts" } },
        ]),
        // Hook runs
        ccLine({
          type: "progress",
