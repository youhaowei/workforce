...
export async function createWorkAgent(deps: LifecycleDeps, config: WorkAgentConfig): Promise<Session> {
  ...
  const session: Session = {
    id: deps.generateId(), title: config.goal, createdAt: now, updatedAt: now,
    parentId: config.parentId, messages: [], metadata,
  };
...
export async function listByState(deps: LifecycleDeps, state: LifecycleState, orgId?: string): Promise<Session[]> {
  await deps.ensureInitialized();
...
export async function getChildren(deps: LifecycleDeps, parentSessionId: string): Promise<Session[]> {
  await deps.ensureInitialized();
