...
function createTestDir(): string {
  const dir = join(
    tmpdir(),
    `workforce-agent-models-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
...
describe("agent-models", () => {
  ...
  describe("ModelCache", () => {
    ...
    it("hydrates from disk cache and returns immediately", async () => {
      ...
      mkdirSync(cacheDir, { recursive: true });
      writeFileSync(
        join(cacheDir, "model-cache.json"),
        JSON.stringify({
          models: diskModels,
          cachedAt: Date.now(),
        }),
    ...
    it("paginates when has_more is true", async () => {
      ...
      mockFetch
        .mockResolvedValueOnce({
          ok: true,
          json: async () => ({
            data: [{ id: "page1", display_name: "Page1" }],
            has_more: true,
            last_id: "page1",
          }),
        })
