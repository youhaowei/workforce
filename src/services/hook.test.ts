...
describe('HookService', () => {
  ...
  describe('registerPreHook', () => {
    ...
    it('throws on duplicate name', () => {
      const svc = getHookService();
      svc.registerPreHook('auth', () => ({ proceed: true }));
      expect(() => svc.registerPreHook('auth', () => ({ proceed: true })))
        .toThrow('Pre-hook already registered: auth');
    });
  ...
  describe('registerPostHook', () => {
    ...
    it('throws on duplicate name', () => {
      const svc = getHookService();
      svc.registerPostHook('log', () => ({}));
      expect(() => svc.registerPostHook('log', () => ({})))
        .toThrow('Post-hook already registered: log');
    });
  });
  ...
  describe('runPreHooks', () => {
    ...
    it('threads modified args through hooks', async () => {
      ...
      svc.registerPreHook('first', (ctx) => {
        order.push('first');
        return { proceed: true, modifiedArgs: { ...ctx.args, added: 'by-first' } };
      }, 10);

      svc.registerPreHook('second', (ctx) => {
        order.push('second');
    ...
    it('short-circuits on proceed:false', async () => {
      ...
      svc.registerPreHook('blocker', () => {
        order.push('blocker');
        return { proceed: false, blockReason: 'denied' };
      }, 10);

      svc.registerPreHook('skipped', () => {
        order.push('skipped');
        return { proceed: true };
      }, 5);
    ...
    it('propagates hook errors to caller', async () => {
      const svc = getHookService();
      svc.registerPreHook('broken', () => { throw new Error('hook failed'); });
  ...
  describe('runPostHooks', () => {
    ...
    it('threads modified results through hooks', async () => {
      ...
      svc.registerPostHook('first', (_ctx, result) => ({
        modifiedResult: `${result}+first`,
      }), 10);

      svc.registerPostHook('second', (_ctx, result) => ({
    ...
    it('collects side effects from all hooks', async () => {
      ...
      svc.registerPostHook('a', () => ({
        sideEffects: [{ event: { type: 'SessionChange', sessionId: 's1', action: 'updated' as const, timestamp: 1 } }],
      }));

      svc.registerPostHook('b', () => ({
        sideEffects: [{ event: { type: 'SessionChange', sessionId: 's2', action: 'updated' as const, timestamp: 2 } }],
      }));
