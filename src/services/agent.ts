/* eslint-disable max-lines -- SDK migration inlined event mapping from unifai; will extract to a separate file post-stabilization. */
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any, complexity -- Flat SDK message dispatch.
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any, complexity -- Flat switch over system subtypes.
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any, complexity -- Flat switch over stream event types.
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any, complexity -- SDK result message extraction.
...
/** Backfill text blocks as streaming events when extended thinking suppressed them. */
function* backfillTextBlocks(content: SdkContentBlock[]): Generator<AgentEvent> {
...
function* extractUserToolResults(msg: SDKMessage, pendingTools: Map<string, string>): Generator<AgentEvent> {
...
/** Flush unresolved pending tools when a new message_start arrives. */
function* flushPendingToolsOnTurnBoundary(msg: SDKMessage, pendingTools: Map<string, string>): Generator<AgentEvent> {
  if (msg.type !== 'stream_event') return;
  const se = msg as SDKMessage & { event?: { type?: string } };
  if (se.event?.type !== 'message_start' || pendingTools.size === 0) return;
...
class SdkSession {
   ...
   */
  // eslint-disable-next-line complexity -- SDK stream loop with tool tracking and backfill.
  async *send(
    message: string,
    options: {
      model?: string;
    ...
    try {
      for await (const msg of stream) {
        ...
        yield* extractUserToolResults(msg, pendingTools);

        // Synthesize tool_result at turn boundaries for unresolved tools
        yield* flushPendingToolsOnTurnBoundary(msg, pendingTools);

        for (const event of mapSdkMessage(msg, includeRaw)) {
          if (event.type === 'message_start') streamedTextThisTurn = false;
          if (event.type === 'text_delta') streamedTextThisTurn = true;

          // Backfill text when stream events were suppressed (extended thinking)
          if (event.type === 'assistant_message' && !streamedTextThisTurn) {
            yield* backfillTextBlocks(event.content);
          }
