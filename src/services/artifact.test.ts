...
async function createSampleArtifact(
  ...
  overrides: Partial<Parameters<ArtifactService["create"]>[0]> = {},
) {
...
describe("ArtifactService", () => {
  ...
  describe("orgId backfill from session JSONL", () => {
    ...
    it("should resolve orgId from linked session header on init", async () => {
      ...
      await writeFile(
        join(artifactsDir, "art_backfill_test.json"),
      ...
      const header = {
        ...
        id: "sess_with_org",
        createdAt: 1,
        metadata: { orgId: "org-resolved" },
      };
      await writeFile(
        join(sessionsDir, "sess_with_org.jsonl"),
    ...
    it("should handle single-line JSONL without trailing newline", async () => {
      ...
      await writeFile(
        join(artifactsDir, "art_noeol.json"),
        JSON.stringify(artifact, null, 2),
        "utf-8",
      );
    ...
    it("should leave orgId as empty string when no session has orgId", async () => {
      ...
      await writeFile(
        join(artifactsDir, "art_noresolve.json"),
        JSON.stringify(artifact, null, 2),
        "utf-8",
      );
