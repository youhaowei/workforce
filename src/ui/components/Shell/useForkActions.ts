...
export function useForkActions({
  ...
  const handleRewind = useCallback(async (messageIndex: number) => {
    ...
    try {
      ...
      const updated = await trpcClient.session.rewind.mutate({ sessionId: sessId, messageIndex: truncateIndex });
      ...
      queryClient.invalidateQueries({ queryKey: trpc.session.forks.queryKey({ sessionId: sessId }) });
