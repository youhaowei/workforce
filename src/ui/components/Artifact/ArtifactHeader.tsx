...
function SingleHeader({
  ...
  return (
    <div className="flex items-center gap-2 px-3 h-10 flex-shrink-0">
      ...
      <span
        className="text-sm font-semibold text-neutral-fg truncate flex-1 font-mono"
        title={filePath}
      >
        {filename}
      </span>
      <span
        className={`text-[10px] px-2 py-0.5 rounded-full font-semibold flex-shrink-0 ${ARTIFACT_STATUS_STYLES[status]}`}
      >
  ...
  return (
    <div className="flex items-end h-10 bg-neutral-bg-subtle flex-shrink-0 overflow-hidden">
        ...
        {artifacts.map((a) => {
          ...
          return (
            ...
            >
              ...
              {isActive && (
                <span
                  className={`text-[8px] px-1.5 py-px rounded font-semibold ${ARTIFACT_STATUS_STYLES[a.status]}`}
                >
      ...
      <Button
        variant="ghost"
        size="icon"
        className="h-7 w-7 flex-shrink-0 mr-1 mb-0.5"
        onClick={onClose}
        title="Close"
      >
        <X className="h-3.5 w-3.5" />
