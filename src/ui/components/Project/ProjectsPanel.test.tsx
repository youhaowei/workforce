...
let capturedDeleteOptions: {
  onMutate?: (input: {
    id: string;
  }) => { wasSelected: boolean; id: string; previousProjects?: Project[] } | undefined;
  onError?: (
    error: unknown,
    vars: { id: string },
    context?: { wasSelected: boolean; id: string; previousProjects?: Project[] },
  ) => void;
} | null = null;
...
describe("ProjectsPanel", () => {
  ...
  it("does not call onSelectProject when Enter is pressed on delete button", () => {
    ...
    render(<ProjectsPanel collapsed={false} onSelectProject={onSelectProject} />);
  ...
  it("restores selection and cache snapshot on delete error", () => {
    ...
    capturedDeleteOptions?.onError?.(
      new Error("delete failed"),
      { id: "proj_1" },
      context ?? undefined,
    );
    expect(mockSetQueryData).toHaveBeenCalledWith(expect.anything(), projectList);
