...
const mockQueryData = {
  data: [] as Array<{ id: string; title: string; status: string; createdAt: number; updatedAt: number }>,
  isLoading: false,
};
...
describe('TaskPanel', () => {
  ...
  it('shows task count badge when tasks exist', () => {
    mockQueryData.data = [
      { id: 'task_1', title: 'Test', status: 'pending', createdAt: Date.now(), updatedAt: Date.now() },
    ];
    render(<TaskPanel isOpen={true} />);
