...
describe("tRPC Routers", () => {
  ...
  describe("session", () => {
    ...
    it("fork rejects non-existent session with NOT_FOUND", async () => {
      await expect(caller.session.fork({ sessionId: "ghost" })).rejects.toThrow(/not found/i);
    });

    it("fork rejects empty session with BAD_REQUEST", async () => {
      const empty = await caller.session.create({ title: "Empty" });

      await expect(caller.session.fork({ sessionId: empty.id })).rejects.toThrow(/empty session/i);
    ...
    it("rewind rejects non-existent session with NOT_FOUND", async () => {
      await expect(caller.session.rewind({ sessionId: "ghost", messageIndex: 0 })).rejects.toThrow(
        /not found/i,
      );
    });
  ...
  describe("worktree", () => {
    it("keep rejects when no worktree exists", async () => {
      ...
      await expect(caller.worktree.keep({ sessionId: session.id })).rejects.toThrow();
