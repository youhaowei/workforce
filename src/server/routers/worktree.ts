...
export const worktreeRouter = router({
  ...
  merge: publicProcedure
    ...
    .mutation(({ input }) => getWorktreeService().merge(input.sessionId, input.strategy)),

  archive: publicProcedure
    .input(z.object({ sessionId: z.string() }))
    .mutation(({ input }) => getWorktreeService().archive(input.sessionId)),

  keep: publicProcedure.input(z.object({ sessionId: z.string() })).mutation(async ({ input }) => {
    ...
    await getOrchestrationService()
      .stopInstance(input.sessionId)
      .catch(() => {
