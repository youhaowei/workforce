...
app.use(
  ...
  cors({
    origin: (origin) => {
      if (!origin) return undefined as unknown as string; // same-origin — no ACAO header needed
      try {
...
function setupStaticServingFromDir(distPath: string) {
  ...
  // @hono/node-server documents `root` as relative to cwd, but path.join
  // handles absolute paths correctly. Electron production always passes an
  // absolute distPath from app.getAppPath().
...
export async function startServer(overrides?: {
  ...
  const { server: httpServer, port } = await bindWithRetry<HttpServer>(
    basePort,
    MAX_PORT_RETRIES,
    (candidate) =>
      serve({
    ...
    .then(
      ...
      (err) =>
        log.warn(
          ...
          "Model cache warm-up failed (will retry on demand)",
        ),
    );
