...
declare const Bun: {
  serve(config: { port: number; fetch: (request: Request) => Response | Promise<Response>; idleTimeout: number }): unknown
}
...
logAuthDiagnostics()
getWorkspaceService().getCurrent().catch((err) => {
  console.error('Failed to initialize workspace service:', err)
})
void getFormDefinitionService().watchEntity('agent-template')
void getFormDefinitionService().watchEntity('workflow-template')
